...
output_dir = fs::path(wd, "../../output/2024/2024_07/20240701/Glu_inhib_coculture2")
```

# load data ----
```{r}
dat = read.csv(path(input_dir, "2024/2024_07/20240628_glu_inhibitor_coculture_formatted.csv")) 
```
...
plot_me = dat %>% 
          ...
          filter(!((Drug == "Cerestat" & Concentration > 90) |
                     ...
                     (Drug == "CPCCOEt" & Concentration > 100)|
                     (Drug == "DMSO" & Concentration > 0.2)|
...
p<-ggplot(plot_me, aes(x = Concentration, 
    ...
    labs(x = str_c("Concentration (DMSO: % v/v; AAA: mM; else uM)"), 
         y = "% Live") +
...
save_plot(path(output_dir, "All_drug_panels/All_drugs_live_zoom3.pdf"), p, base_aspect_ratio = 4)
...
plot_me = dat %>%
          ...
          mutate(avg = mean(Proliferated),
                 ...
                 Drug2 = ifelse(grepl("None", Drug),"None",
                                ifelse(Drug == "DMSO" & Concentration < 0.01,
                           str_c(Drug, " (", round(Concentration, 3), Unit, ")"),
                           ifelse(Drug == "Glutamate" & Concentration <= 0.001,
                                str_c(Drug, " (", Concentration, Unit, ")"),
                                str_c(Drug, " (", round(Concentration,2), Unit, ")"))))) %>%
...
p<-ggplot(plot_me2, 
  ...
  geom_hline(#data = plot_me2 %>% filter(Drug2 == "None (Treated)"),
             aes(yintercept = 70.56), linetype =2) +
  scale_fill_manual(values = c("OVA" = "white",
                                "LSG+OVA" = "#F68064")) +
  scale_alpha_manual(values = c("6" = 0.2, "5" = 0.4,"4" =0.5,"3" = 0.7, "2" = 0.85, "1" = 1)) +
  facet_wrap(~Drug, scales="free_x", nrow=1) +
  labs(x = NULL, y = "% Prolfierated") +
  theme(axis.text.x =element_text(angle =45,hjust =1),
        legend.position = "none") +
  guides(alpha = "none")

p
save_plot(path(output_dir, "PROLIFERATION_all_drugs_barplot_v2.pdf"), p,
          base_aspect_ratio = 4)
