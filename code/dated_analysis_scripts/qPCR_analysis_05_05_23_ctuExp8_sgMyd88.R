...
#read in data
qPCR_dat = read.csv(fs::path(input_dir, "2023-05-05_ctuExp8_myd88-1_fromatted.csv")) %>%
           data.frame() %>%
...
write.csv(as.matrix(dCq_dat), path(wd, "output/2023/2023_05/2023_05_04/2023_05_05_ctuExp8_myd88.csv"))
...
p1<-make_Cq_plot_bar("Myd88", myd88_dat) + 
     scale_fill_manual(values =c( Scr = "grey",
                                  ...
                                  Ctu1 = "#8C9FCA",
                                  Ctu2 = "#F68064")) +
     labs(title = "Myd88 Expression")

p2<-make_Cq_plot_bar("Gapdh", myd88_dat) + 
      scale_fill_manual(values =c( Scr = "grey",
                                   Myd88 = "#68C3A5" ,
                                   Untransduced = "white",
                                   Empty = "grey90",
...
p3 <- make_Cq_plot_bar("Myd88", myd88_dat, dCq = T) + 
      scale_fill_manual(values =c( Scr = "grey",
                               Myd88 = "#68C3A5" ,
                               Untransduced = "white",
                               Empty = "grey90",
                               Ctu1 = "#8C9FCA",
                               Ctu2 = "#F68064")) +
      labs(title = "Delta Cq (Myd88 - Gapdh)")

save_plot(path(output_dir, "Myd88_KD/Myd88_cq.pdf"), p1)
save_plot(path(output_dir, "Myd88_KD/Gapdh_cq-Myd88KD.pdf"), p2)
save_plot(path(output_dir, "Myd88_KD/Myd88-Gapdh_dcq.pdf"), p3)
...
egg_data <- data.frame(
  Nutrient = c("Water", "Protein", "Fat", "Minerals"),
  Egg_Part = c("Egg White", "Egg White", "Egg White", "Egg White", "Egg Yolk", "Egg Yolk", "Egg Yolk", "Egg Yolk"),
  Percentage = c(88.0, 11.0, 0.2, 0.8, 48.0, 17.5, 32.5, 2.0)
)

p<-ggplot(data = egg_data, aes(x = Egg_Part, y = Percentage, fill = Nutrient)) +
  geom_bar(stat = "identity", position = "stack"15) +
  labs(title = "Molecules in Egg",
       x = "Egg Part",
       y = "Percentage") +
...
save_plot("egg_composition.pdf", p)
