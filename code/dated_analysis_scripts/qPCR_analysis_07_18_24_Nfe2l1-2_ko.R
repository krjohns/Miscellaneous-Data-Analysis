...
#set directories
wd = getwd()
input_dir = path(wd, "input/data/2024/2024_07")

#read in data
qPCR_dat = read.csv(fs::path(input_dir, "20240716_Nfe2l1-2ko_qPCR.csv")) %>%
           data.frame() %>%
           select(Date, Well, Sample, Target, Treatment, Gene, Bio_Rep, Cq) %>%
           #add columns for qpcr gene, biol rep, tech rep
           mutate(Tech_Rep = ifelse(as.numeric(gsub("[A-P]","", Well)) %% 2 != 0, "1", "2")) %>%
           #drop unreliable data
           #filter(!(Date == "7_16_24" & Target == "Ctrl" & Bio_Rep == 2)) %>%
           filter(!(Target == "Ctrl" & Bio_Rep == 2)) %>%
           filter(!(grepl("Nfe2l1_sgNfe2l2|pool", Sample) & Bio_Rep == 1)) %>%
           filter(!(is.na(Cq) | Cq > 30)) 
           
            
dat = qPCR_dat %>%  
           dplyr::select(-c(Well, Date)) %>%  #drop well info for pivoting
...
Gapdh = dat %>%
        filter(Gene == "Gapdh") %>%
        select(Bio_Rep, Sample, Target, Treatment, Avg_Cq) %>%
        rename_at(vars(Avg_Cq), ~str_c("Gapdh_", .))#record Gapdh genes for merging

dCq_dat =  dat %>%
           select(Gene, Bio_Rep, Sample, Tech_Rep_1, Tech_Rep_2,
                  Treatment, Target, Avg_Cq, `2_Avg_Cq`) %>%
           filter(Gene != "Gapdh") %>% # remove Gapdh rows
           merge(Gapdh, 
                 by = c("Bio_Rep", "Target", "Treatment","Sample")) %>% # merge with Gapdh data
...
means = dCq_dat %>%
        group_by(Gene, Sample, Target, Treatment) %>%
...
dCq_dat = dCq_dat %>%
          full_join(means, by = c("Gene", "Sample","Target", "Treatment")) 


#write.csv(as.matrix(dCq_dat), path(wd, "output/2024/2024_07/20240719/20240723_Nfe2l1-2ko_qPCR_dat_filt_additional_targets.csv"))
...
plot_me <- dCq_dat %>%
           ...
           mutate(Pool = grepl("pool", Sample),
                  Gene = factor(Gene, levels = c("Nfe2l1", "Nfe2l2", "Slc7a11", "Gclc", "Gls")),
                  Target = factor(Target, levels =c("Ctrl", "Nfe2l1", "Nfe2l2", "Nfe2l1+Nfe2l2"))) %>%
           arrange(Target, Pool, Sample)
...
save_plot(path(wd, "output/2024/2024_07/20240719/20240723_legend.pdf"), p,
