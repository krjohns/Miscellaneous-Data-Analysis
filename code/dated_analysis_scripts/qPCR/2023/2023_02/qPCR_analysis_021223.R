...
#read in data
qPCR_dat = read.csv(fs::path(input_dir, "2023_02_14_ctu_repeat_1to10_formatted.csv")) %>%
           data.frame() %>%
           #add columns for qpcr gene, biol rep, tech rep
           mutate(Gene = ifelse(grepl("[A-B;E-F]", Well), "Ctu1",
                                ifelse(grepl("[C-D;G-H]", Well), "Ctu2", 
                                ifelse(grepl("[I-N]", Well), "Gapdh", "Il-6"))),
                  Bio_Rep = ifelse(grepl("[A;C;E;G;I;K;M:O]", Well), "1", "2"),
                  Tech_Rep = ifelse(as.numeric(gsub("[A-P]","", Well)) %% 2 != 0, "1", "2"),
                  #re-format oligo column
                  Oligo = gsub("s", "sh",gsub("_myd88", "1",gsub("none", "",gsub("_scr_", "", Oligo)))),
...
dCq_dat =  dat %>%
           ...
           merge(Gapdh, by = c("Biol_Rep", "Target_Oligo", "Gene")) %>% # merge with Gapdh data
