...
#read in data
qPCR_dat = read.csv(fs::path(input_dir, "2023_04_04_il6_virus_testing_bmdc_sgMyd88_formatted.csv")) %>%
           data.frame() %>%
...
write.csv(as.matrix(dCq_dat), path(wd, "output/2023/2023_04_04/2023_04_04_myd88_virus_vector_testing_filtoutlier.csv"))
...
ggarrange(plotlist=list(make_Cq_plot("Il6", il6_plot, colors = `Virus prep`) + 
                            scale_fill_manual(values =c("#F68064", "#8C9FCA")) +
                            scale_color_manual(values =c("#F68064", "#8C9FCA")) +
                            geom_hline(yintercept = 2^-24, color = "black", linetype = 2) +
                            geom_hline(yintercept = 2^-19, color = "black", linetype = 2) +
                            facet_wrap(~Vector, nrow =1, scales = "free_x"),
                        make_Cq_plot("Gapdh", il6_plot, colors = `Virus prep`) +
                           scale_color_manual(values =c("#F68064", "#8C9FCA")) +
                           scale_fill_manual(values =c("#F68064", "#8C9FCA")) +
                           geom_hline(yintercept = 2^-21, color = "black", linetype = 2) +
                           geom_hline(yintercept = 2^-20.5, color = "black", linetype = 2) +
                           geom_hline(yintercept = 2^-20, color = "black", linetype = 2) +
                           geom_hline(yintercept = 2^-19.5, color = "black", linetype = 2) +
                           ylim(c(1e-07, 1.5e-06)) +
                           facet_wrap(~Vector, nrow =1, scales = "free_x"),
                        make_Cq_plot("Il6", il6_plot, colors = `Virus prep`, dCq=T) +
                          scale_fill_manual(values = c("#F68064", "#8C9FCA")) +
                          scale_color_manual(values =c("#F68064", "#8C9FCA")) +
                          facet_wrap(~Vector, nrow=1, scales = "free_x")),
            #ncol = 1,
...
save_plot(path(wd, "output/2023/2023_04_04/il6_myd88_virus_vector_testing_NO_TITLE.pdf"), p)
