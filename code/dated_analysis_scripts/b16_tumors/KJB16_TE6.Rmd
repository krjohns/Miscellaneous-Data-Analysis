...
# load data ----
```{r}
dat = read_excel(path(input_dir, "20241218_b16_tumor_exp6.xlsx"), sheet = "measurement_log") 
...
my_comps = list("SCR-L-OVA" = "OVA|SCR-L", "CTC1.1-L" = "CTC|SCR-L",
                "CTC1.1-OVA" = "CTC|SCR-OVA")

#my_comps = list("CTC1.1-L" = "CTC|SCR-L",
            #    "CTC1.1-OVA" = "CTC|SCR-OVA")
...
plot_me = dat %>% 
          ...
          mutate(Minor = as.numeric(X_Dimension) < as.numeric(Y_Dimension),
                 ...
                 Condition = factor(Condition, levels = c("SCR-OVA", "SCR-L",
                                                          "CTC1.1-L"))) %>%
           filter(Day <= day) %>%
           filter(Mouse_ID != "1_6369110M4*") %>%
           filter(Cage_card != "6369109")
...
for(j in (1:length(p_list))){
  save_plot(path(output_dir, str_c("TE", exp,"/d", as.character(day),"/d", as.character(day),"_cage_dropped/",
