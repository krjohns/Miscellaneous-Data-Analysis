...
output_dir = path(wd, "output/2023/2023_06/2023_06_28")

#read in data
qPCR_dat = read.csv(fs::path(input_dir, "2023_06_28_ctuExp12_formatted.csv")) %>%
           data.frame() %>%
...
write.csv(as.matrix(dCq_dat), path(wd, "output/2023/2023_06/2023_06_28/2023_06_28_ctuExp12_dCqTable.csv"))
...
dat = dCq_dat %>% 
           filter(Gene != "Ctu2") %>%
           filter(Gene != "Ctu1") %>%
           filter(Target %in% c("Empty","Scr","Myd88")) %>%
...
p3 <- make_Cq_plot_bar("Myd88", dat, dCq = T) + 
      scale_fill_manual(values =c( Scr = "grey",
                               Untransduced = "white",
                               Empty = "grey90",
                               Myd88 = "#68C3A5",
...
save_plot(path(output_dir, "Myd88_KD/Myd88_cq_only.pdf"), p1)
save_plot(path(output_dir, "Myd88_KD/Gapdh_cq-Myd88KD_only.pdf"), p2)
save_plot(path(output_dir, "Myd88_KD/Myd88-Gapdh_dcq_only.pdf"), p3)

#ggplot(dat)


p<-ggplot(dat, aes(x = Target_oligo, y = dCq))

p +
  geom_bar(stat = "identity", aes(fill = Bio_Rep)) +
