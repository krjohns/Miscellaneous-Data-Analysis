...
# load data ----
```{r}
dat = read_excel(path(input_dir, "20241113_b16_tumor_exp2.xlsx"), sheet = "measurement_log") 
...
```{r}
day = 23
my_comps = list("SCR-OVA-LSG" = "OVA|SCR-LSG", "SCR-OVA-Ctu2g1" = "OVA|Ctu2_g1",
...
for(i in (1:length(my_comps))){#calc tumor volume (.5 * D * d^2) D = Y, d = X
  plot_me = dat %>% 
          select(Mouse_ID, Marking, Day, Group, Condition, X_Dimension, Y_Dimension,
                 calc_vol) %>%
          filter(!((Group == "2" & Marking == 0)| (Group == "3" & Marking == 4))) %>%
          filter((grepl(my_comps[[i]], Condition))) %>%
          mutate(Mouse_ID = ifelse(Mouse_ID == "NA", Marking, Mouse_ID),
                 Minor = as.numeric(X_Dimension) < as.numeric(Y_Dimension),
...
p_base <-ggplot(plot_me, aes(x = Day, y = Volume, color = Condition))+
         ...
         labs(x = "Time (Days)", y = "Tumor Volume (mm<sup>3</sup>)", title = "KJ B16-TE-2") +
...
for(j in (1:length(p_list))){
  save_plot(path(output_dir, str_c("TE2/d", as.character(day),"/",
                names(my_comps)[[i]], "/tumor_vol_TE2-",names(my_comps)[[i]],
                names(p_list)[j], ".pdf")),
