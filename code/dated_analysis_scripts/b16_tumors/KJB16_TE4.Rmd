...
# load data ----
```{r}
dat = read_excel(path(input_dir, "20241204_b16_tumor_exp4.xlsx"), sheet = "measurement_log") 
...
#my_comps = list("SCR-OVA-LSG" = "OVA|SCR-LSG", "B16-OVA" = "OVA|B16",
          #      "B16-SCR-LSG" = "SCR-LSG|B16","SCR-OVA-Ctu2g1" = "SCR-OVA|Ctu2_g1",
...
for(i in (1:length(my_comps))){#calc tumor volume (.5 * D * d^2) D = Y, d = X
  plot_me = dat %>% 
          ...
          mutate(Condition = ifelse(Condition != "B16_only", 
                                    str_c("sg", Condition), Condition)) %>%
          ...
          filter(Day <= day) %>%
          #drop mouse that died
          filter(!(Mouse_ID == "1_6365684M3"))
...
p_base <-ggplot(plot_me, aes(x = Day, y = Volume, color = Condition))+
         ...
         labs(x = "Time (Days)", y = "Tumor Volume (mm<sup>3</sup>)", title = "KJ B16-TE-4") +
...
for(j in (1:length(p_list))){
  save_plot(path(output_dir, str_c("TE4/d", as.character(day),"/",
                names(my_comps)[[i]], "/tumor_vol_TE4-",names(my_comps)[[i]],
                names(p_list)[j], ".pdf")),
