...
#read in data
qPCR_dat = read.csv(fs::path(input_dir, "kj_ctu_kdko_exp2_controlBMDCs_022023_formatted.csv")) %>%
           data.frame() %>%
...
write.csv(as.matrix(dCq_dat), path(wd, "output/2023/2023_02_22_UnstimBMDC_dCqTable.csv"))
...
p <- ggarrange(plotlist=list(
  ggarrange(plotlist = list(make_Cq_plot("Ctu2") + 
...
save_plot(path(wd, "output/2023/ctu2_untreated_zoom_newcolors.pdf"), save_me)
