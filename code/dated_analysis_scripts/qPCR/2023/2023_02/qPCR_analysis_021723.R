...
#read in data
qPCR_dat = read.csv(fs::path(input_dir, "kj_ctu_kdko_exp2_LPSstim_021723.csv")) %>%
           data.frame() %>%
...
write.csv(as.matrix(dCq_dat), path(wd, "output/2023/2023_03_02/2023_03_02_dCqTable.csv"))

#make plots ----
make_Cq_plot = function(gene, dat, dCq = F){
...
full_dCq_dat <- dCq_dat
dCq_dat <- dCq_dat %>%
  filter(!(Target_oligo %in% c("None", "Scr_sh2", "Scr_sh3")))


p <- ggarrange(plotlist=list(
  ggarrange(plotlist = list(make_Cq_plot("Ctu2", dCq_dat) + 
                              scale_color_manual(values =c('Control' = "#F68064",
                                                           "Ctu1" = "#68C3A5",
                                                           "Ctu2" = "#8C9FCA")),
                            make_Cq_plot("Gapdh") +
                              scale_color_manual(values =c('Control' = "#F68064",
  ...
  align= "hv") 
#8C9FCA" #B9CEE6"
save_me <- annotate_figure(p, top = text_grob("Ctu2 Expression (+LPS)"))

save_plot(path(wd, "output/2023/ctu2_lps_treated_edited.pdf"), save_me)
