...
# load data ----
```{r}
dat = read_excel(path(input_dir, "20250117_b16_tumor_exp8.xlsx"), sheet = "measurement_log") 
...
plot_me = dat %>% 
          ...
          mutate(Minor = as.numeric(X_Dimension) < as.numeric(Y_Dimension),
                 ...
                 Mouse_ID = str_c(Group, "_", Cage_card,"M", Mouse_ID), #Cage_card,
                 Day = as.numeric(gsub("d", "", Day)),
                 Volume = 0.5*D1*(d1)^2,
                 Tumor_leak = ifelse(grepl("22M0|22M1|22M3",Mouse_ID),T,F)
...
p_base<-ggplot(plot_me, aes(x = Day, y = Volume, color = Condition))+
        scale_color_manual(values = c("B16-OVA" = "grey40",
                                      "SCR-OVA" ="black",
                                      "SCR-LPS+OVA" = "#B9CEE6",
                                      "CTC1.1-L" = "#68C3A5",
                                      "TTC1.3-L" ="#539C84",
...
p_indiv<-p_base +
         geom_line(aes (linetype = Tumor_leak, #as.character(Cage_card) 
                        group = Mouse_ID), 
...
for(j in (1:length(p_list))){
  save_plot(path(output_dir, str_c("TE", exp,"/d", as.character(day),"/d", as.character(day),"_by_leak/",
                names(my_comps)[[i]], "/tumor_vol_TE", exp, "-",names(my_comps)[[i]],
                names(p_list)[j], ".pdf")),
                p_list[[j]], base_aspect_ratio =1.45)
  }
