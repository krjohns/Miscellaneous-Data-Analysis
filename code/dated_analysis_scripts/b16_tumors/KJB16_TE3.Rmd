...
# load data ----
```{r}
dat = read_excel(path(input_dir, "20241126_b16_tumor_exp3.xlsx"), sheet = "measurement_log") 
...
```{r}
day = 22
my_comps = list("SCR-L-OVA" = "OVA|SCR-L", "CTC1.1-L" = "CTC|SCR-L",
                "CTC1.1-OVA" = "CTC|SCR-OVA", "TTC1.3-L" = "TTC1.3|SCR-L", 
                "TTC1.3-OVA" = "TTC1.3|SCR-OVA", "TTC2.1-OVA" = "TTC2.1|SCR-OVA", 
                "TTC2.1-L" = "TTC2.1|SCR-L")

for(i in (1:length(my_comps))){
#calc tumor volume (.5 * D * d^2) D = Y, d = X
plot_me = dat %>% 
          select(Mouse_ID, Marking, Day, Group, Condition, X_Dimension, Y_Dimension,
                 calc_vol) %>%
          ...
          mutate(Minor = as.numeric(X_Dimension) < as.numeric(Y_Dimension),
                 ...
                 Volume = 0.5*D1*(d1)^2,
                 Condition = factor(Condition, levels = c("SCR-OVA", "SCR-L",
                                                          "CTC1.1-L", "TTC1.3-L",
                                                          "TTC2.1-L"))) %>%
           filter(Day <= day)
...
p_base<-ggplot(plot_me, aes(x = Day, y = Volume, color = Condition))+
        scale_color_manual(values = c("SCR-OVA" ="black",
                                      ...
                                      "TTC1.3-L" ="#539C84",
                                       "TTC2.1-L" = "#3E7563")) +
        labs(x = "Time (Days)", y = "Tumor Volume (mm<sup>3</sup>)", title = "KJ B16-TE-3") +
...
for(j in (1:length(p_list))){
  save_plot(path(output_dir, str_c("TE3/d", as.character(day),"/",
                names(my_comps)[[i]], "/tumor_vol_TE3-",names(my_comps)[[i]],
                names(p_list)[j], ".pdf")),
                p_list[[j]], base_aspect_ratio =1.3)
  }
