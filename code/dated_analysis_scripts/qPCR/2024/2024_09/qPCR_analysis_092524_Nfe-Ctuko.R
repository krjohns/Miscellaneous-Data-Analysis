...
#set directories
wd = getwd()
input_dir = path(wd, "input/data/2024/2024_09")

#read in data
qPCR_dat = read.csv(fs::path(input_dir, "20240925_ctu_nfe_KO_unstim_compiled.csv")) %>%
           data.frame() %>%
           select(Well, Sample, Gene, Tech_Rep, Bio_Rep, Cq) %>%
           #add columns for qpcr gene, biol rep, tech rep
           mutate(Tech_Rep = ifelse(Gene == "Ctu1" | Gene == "Ctu2",
                            Tech_Rep, 
                            ifelse(as.numeric(gsub("[A-P]","", Well)) %% 2 != 0, "1", "2"))) %>%
           #drop unreliable data
           filter(!(is.na(Cq))) %>%
           filter(!(Gene == "Gapdh" & Cq > 30 | (Gene == "Nfe2l2" & Cq > 28) | (Gene == "Ctu2" & Cq > 35)))
...
write.csv(as.matrix(dCq_dat), path(wd, "output/2024/2024_09/20240925/20240925_Ctu-NfeKO_qPCR_dat.csv"))

plot_me <- dCq_dat %>%
           mutate(Target = gsub("-.*", "", Sample),
                  Target = gsub("sgNfe2l1_sgNfe2l2", "Double KO", Target)) %>%
           filter(!(grepl("Nfe|Slc|Gcl", Gene))) %>%
           select(Gene, Sample, Target, rep_means_2_dCq, rep_se_2_dCq) %>%
           #remove individual replicate rows
           unique() %>%
           mutate(Gene = factor(Gene, levels = c("Nfe2l1", "Nfe2l2", "Gclc","Slc7a11", "Ctu1", "Ctu2")),
                  Target = factor(Target, levels =c("sgCtrl", "sgNfe2l1", "sgNfe2l2","Double KO", "sgCtu1", "sgCtu2"))) %>%
           arrange(Target, Sample)

plot_me2 <- plot_me %>%
            mutate(Sample = factor(Sample, levels = unique(plot_me$Sample))) %>%
            filter(Sample != "sgCtrl-6") %>%
            filter(Sample != "sgNfe2l2-2") %>%
            filter(Sample != "sgNfe2l1_sgNfe2l2-2") %>%
            filter(Target != "sgCtu1")
...
p
save_plot(path(wd, "output/2024/2024_09/20240925/20240925_Ctu2.pdf"), p,
          base_aspect_ratio = 2)
