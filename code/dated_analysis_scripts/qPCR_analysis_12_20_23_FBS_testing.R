...
#set directories
wd = getwd()
input_dir = path(wd, "input/data/2023/2023_12")
output_dir = path(wd, "output/2023/2023_12/2023_12_20")

#read in data
qPCR_dat = read.csv(fs::path(input_dir, "2023-12-19_FBS_testing_imBMDM_KJ_formatted.csv")) %>%
           data.frame() %>%
...
Gapdh = dat %>%
        filter(Gene == "Gapdh") %>%
        select(Bio_Rep, Experiment, Treatment, Media_type, Avg_Cq) %>%
        rename_at(vars(Avg_Cq), ~str_c("Gapdh_", .))#record Gapdh genes for merging

dCq_dat =  dat %>%
           select(Gene, Bio_Rep, Tech_Rep_1, Tech_Rep_2,
                  Experiment, Treatment, Media_type, Avg_Cq, `2_Avg_Cq`) %>%
           filter(Gene != "Gapdh") %>% # remove Gapdh rows
           merge(Gapdh, by = c("Bio_Rep", "Experiment", "Treatment", "Media_type")) %>% # merge with Gapdh data
...
means = dCq_dat %>%
        group_by(Gene, Experiment, Treatment, Media_type) %>%
...
dCq_dat = dCq_dat %>%
          full_join(means, by = c("Gene", "Experiment", "Treatment", "Media_type")) #%>%
...
write.csv(as.matrix(dCq_dat), path(output_dir, "2023_12_20_FBS-test-imBMDM_dCqTable.csv"))
...
dat = dCq_dat %>% 
      select(Gene, Treatment, Experiment, Media_type, rep_means_2_dCq, rep_se_2_dCq) %>%
      unique() %>%
      filter(Experiment == "FBS_testing")
...
p<-ggplot(dat, aes(x = Treatment, y = rep_means_2_dCq)) +
  ...
  scale_fill_manual(values = c("grey", "#8C9FCA", "#539C84")) +
...
save_plot(path(output_dir, "FBS_testing.pdf"), p)
